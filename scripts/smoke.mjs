...
try {
  ...
  assert.equal(await js('window.quickPaste.getData().then(data => data.settings.hotkeyMode)'), 'toggle')
  await js('document.querySelectorAll(\'input[name="hotkey-mode"]\')[1].click()')
  await until(() => js('window.quickPaste.getData().then(data => data.settings.hotkeyMode === "hold")'), 'hold hotkey mode')
  await js('document.querySelectorAll(\'input[name="hotkey-mode"]\')[0].click()')
  await until(() => js('window.quickPaste.getData().then(data => data.settings.hotkeyMode === "toggle")'), 'toggle hotkey mode')
  ...
  await writeFile(resolve(directory, 'report.json'), JSON.stringify({ ok: true, checks: ['onboarding', 'samples', 'fixed-wheel', 'draft-confirm', 'draft-across-settings', 'update-settings', 'hotkey-mode-setting', 'export', 'import-preview', 'duplicate-skip', 'minimum-window-width', 'restore-backup', 'draft-after-reload'], screenshotDirectory: directory }, null, 2))
