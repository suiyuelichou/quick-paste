...
公开发布后，在 [Releases 页面](https://github.com/suiyuelichou/quick-paste/releases) 下载 `Quick-Paste-1.3.0-x64.exe`。如果当前还没有公开 Release，可以按照下方开发步骤自行打包。

安装后从开始菜单启动 Quick Paste，首次打开会进入“一分钟上手”。关闭管理窗口后仍驻留托盘，从托盘菜单选择“退出”才会结束程序。

## 功能

- **鼠标旁的轮盘：**默认 `Ctrl+Alt+Space` 唤起，每页 8 条，滚轮翻页，方向键选择、Enter 输入、Esc 退出；可选择按一下打开或仅在按住时显示。
...
2. 点击练习框或其他普通应用的输入框，按 `Ctrl+Alt+Space`，点击轮盘中的文本；也可以在设置中改为按住组合键显示、松开关闭。
3. 在文本库中编辑内容，拖动分组和条目安排轮盘位置。轮盘标签自动取正文第一行前 14 个字符。

关闭管理页后应用仍驻留系统托盘。快捷键、唤起方式、开机启动和应用更新可在“设置”中管理。“按一下打开”适合搜索和键盘导航；“按住显示”会在松开组合键中的任一按键时关闭，更适合鼠标选择。更新检查仅访问 GitHub Releases，不会上传本地文本；下载完成后由用户选择是否立即重启安装。为避免影响其他软件的焦点切换，快捷键必须包含修饰键，不会占用单独的 `Tab` 键。
...
需要 Windows 和 Node.js 22.12+，建议使用 Node.js 22 LTS。`npm run native:build` 使用 Windows .NET Framework C# 编译器构建 `native/InputHelper.cs`。安装包输出到 `dist/Quick-Paste-1.3.0-x64.exe`。
...
- 数据以原子写入的 JSON 文件保存在 Electron `userData` 目录中，不会上传。“按住显示”仅在轮盘唤起期间检查所配置组合键是否仍按下，不记录其他按键。
